...
pal <- c("0" = "#ffffff",   # white for 0-case states (now shows in legend)
         "1" = "#fef0d9",
         "2" = "#fdcc8a",
         "3" = "#fc8d59",
         "4" = "#d7301f",
...
ggplot(map_merged) +
  geom_sf(aes(fill = severity_bucket),
          colour = "black", size = 0.4) +                    # black borders
  scale_fill_manual(
    values   = pal,
    na.value = "grey90",
    drop     = FALSE,
  ...
  labs(
    title   = "Measles Severity by State (per 100 K) up to 21 June 2025",
    caption = "Note: Only states with severity ≥ 2 cases per 100K are labelled for emphasis."
  )
```
